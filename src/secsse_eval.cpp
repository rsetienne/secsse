                    ...
                    size_t num_steps)
{
  ...
  if (rhs == "ode_standard") {
    ...
    ? eval<odeintcpp::no_normalization>(std::make_unique<ode_standard<OdeVariant::complete_tree>>(ll, mus, Q), ances, states, forTime, method, atol, rtol, num_steps)
      : eval<odeintcpp::no_normalization>(std::make_unique<ode_standard<OdeVariant::normal_tree>>(ll, mus, Q), ances, states, forTime, method, atol, rtol, num_steps);
  } 
  else if (rhs == "ode_cla") {
    auto ll = Rcpp::as<Rcpp::List>(lambdas);
    return is_complete_tree 
    ? eval<odeintcpp::no_normalization>(std::make_unique<ode_cla<OdeVariant::complete_tree>>(ll, mus, Q), ances, states, forTime, method, atol, rtol, num_steps)
      : eval<odeintcpp::no_normalization>(std::make_unique<ode_cla<OdeVariant::normal_tree>>(ll, mus, Q), ances, states, forTime, method, atol, rtol, num_steps);
  }
  else {
