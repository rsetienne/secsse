...
# ideally, one should pick random initial parameters,
# but here, we will reduce computation time by starting in (the otherwise
# unknown) true parameters
# initpars <- rexp(n = max(trans_matrix, na.rm = TRUE), 10)
initpars <- params
...
fit_model <- function(tree, traits, model) {
  ...
  initpars <- rexp(n = max_indicator, 10)
