...
#' and data using Rcpp, using different parameters before and after a 
#' critical timepoint.
#' @title Likelihood for SecSSE model, using Rcpp
#' @param parameter list where the first is a table where lambdas across
#' different modes of speciation are shown (before critical t), 
#' the second mus and the third transition rates, all before the critical time
#' point. Then, the fourth entry is the lambdas across different modes of 
#' speciation, but after the critical t. Similarly, the fifth and sixth entries
#' are the mus and transition rates respectively, again after the critical
#' timepoint.
...
#'# LL = -42.18407
#' @export
cla_secsse_loglik_timezones_compound <- function(parameter,
                                          phy,
                                          traits,
  ...
  if (num_threads == 1) {
    ...
    calcul <- cla_calThruNodes_timezones_compound_cpp(ancescpp,
                                             states,
                                             forTimecpp,
                                             parameter,
                                             critical_t,
  ...
  if (is.numeric(root_state_weight)) {
    ...
    if (root_state_weight == "proper_weights") {
      ...
      for (j in 1:lmb) {
        numerator[j] <- mergeBranch2[j] / sum(lambdas1[[j]] *
  ...
  if (cond == "proper_cond") {
    preCond <- rep(NA, lmb) # nolint
    for (j in 1:lmb) {
      preCond[j] <- sum(lambdas1[[j]] * ((1 - nodeM[1:d]) %o% (1 - nodeM[1:d])))
    }
