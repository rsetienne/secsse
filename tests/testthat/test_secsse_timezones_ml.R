context("test_secsse_ml_timezones")
...
test_that("trying a short ML search: secsse_ml & parallel procedure", {
  ...
  critical_t <- c(0.5)
  testthat::expect_output(
    model_timezones <- secsse_timezones_ml(
      phy = phylotree,
      traits = traits,
      num_concealed_states = num_concealed_states,
      idparslist = idparslist_t,
      idparsopt = idparsopt,
      initparsopt = initparsopt,
      idparsfix = idparsfix,
      parsfix = parsfix,
  ...
  testthat::expect_equal(model$ML, model_timezones$ML)
})  