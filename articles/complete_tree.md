...
Most current studies of evolutionary dynamics make use of molecular
...
estimation with secsse.

Note that here “complete tree” should not be taken as a complete
...
we are going to simulate phylogenies with
[`secsse_sim()`](https://rsetienne.github.io/secsse/reference/secsse_sim.md).
We will simulate a reconstructed and a complete version of the same tree
under the CR model.
...
example to that of the *Simulating with secsse* vignette. For more
details on this model and full details of the functionality of
[`secsse_sim()`](https://rsetienne.github.io/secsse/reference/secsse_sim.md),
see
...
![](complete_tree_files/figure-html/sim_plot_tree-1.png)

## Fitting the model

Finally, we run
[`secsse_ml()`](https://rsetienne.github.io/secsse/reference/secsse_ml.md)
on our complete tree, much in the same way as we would for one with
extant species. However, this time we make sure to set the
`is_complete_tree` argument to `TRUE` (defaults to `FALSE` if omitted).
...
It would be interesting to see how they compare with the same tree
...
![](complete_tree_files/figure-html/fitting_ml_reconstructed_tree-1.png)
...
state 1 to 0 also improved.

As a final note, do note that this is just a simple simulation example
and care should be exercised with model selection and specification when
fitting secsse to empirical datasets to make predictions about
evolutionary patterns.

## References

Nee S, May RM, Harvey PH. The reconstructed evolutionary process. Philos
Trans R Soc Lond B Biol Sci. 1994 May 28;344(1309):305-11.
