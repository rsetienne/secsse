...
A good test of the fit of your secsse model, is to verify found
...
Tree simulation in secsse takes a very similar form to performing a
...
For a more detailed description of how the Lambda List, Mu vector and Q
matrix work, we refer to the vignette
...
In order for secsse to be able to use these to simulate a tree, we need
to provide actual starting parameters. secsse has a helping function
...
will go over the different objects and fill in the appropriate parameter
value from the `used_params` vector, e.g. when it finds a `1` as rate
indicator, it enters the value at position `used_params[1]`, when it
encounters a `2` as rate indicator, it enters the value at position
`used_params[2]` etc.
...
![](sim_with_secsse_files/figure-html/simulate_tree-1.png)

### Conditioning

Notice that
[`secsse_sim()`](https://rsetienne.github.io/secsse/reference/secsse_sim.md)
can simulate a tree conditioning on different tip-states: either it uses
the conditioning `obs_states`, in which case secsse will keep simulating
until it simulates a tree that has all observed states. This is usually
...
Alternatively, secsse can also condition on `true_states` - in this case
secsse will try to simulate until all possible combinations of observed
...
```
    ...
    ##  [1] "1B" "1A" "1A" "0A" "1B" "1B" "1B" "1B" "1B" "1B" "1B" "0A" "0A" "0A" "0A"
...
```
    ...
    ##  [1] "1A" "1A" "0A" "0A" "0A" "1A" "1A" "1A" "1A" "1A" "1A" "1A" "0A" "1A" "1A"
    ## [16] "0A" "1A" "1A" "1A" "0A" "0A" "0A" "1A" "1B" "1A" "0A" "0A" "0A" "0A" "0A"
    ## [31] "0A" "0A" "0A" "0B" "0B" "0A" "0A" "0B" "0B" "1A" "0A" "0A" "1A" "1A" "0A"
    ## [46] "0A" "0B" "0B" "0A" "1A" "1A" "1A" "1A"

Here, we have only explored a two-state system and the differences may
not be very large, but for large numbers of states, such conditioning
might yield very different trees.
